...
from sheet_layouts import MIP_LAYOUT
...
def run(job_dir, template_path, tolerance_profile_path, output_dir, unit):
    ...
    snapshot_path = output_dir / "MIP_snapshot.png"
    ...
    snapshot = snapshot_workbook(
        ...
        sheet_name=MIP_LAYOUT.sheet_name,
        min_rows=26,
        min_cols=MIP_LAYOUT.last_column,
    )
...
def parse_args(argv=None):
    ...
    parser = argparse.ArgumentParser(
        description="Fill and snapshot the MIP sheet from ocr_results.json."
    )
    ...
    parser.add_argument(
        "--output-dir",
        help="Defaults to <job>/excel-output.",
    )
    parser.add_argument(
        "--unit",
        choices=("metric", "inch"),
