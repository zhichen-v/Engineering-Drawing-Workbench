...
7. 「儲存標註資料」會輸出各頁 snapshot 與 `boxes.json`。
...
網頁中的「執行辨識」會先更新目前文件所有頁面的 crop 與 `boxes.json`，再執行 OCR。完成後可切換 OCR 標註顯示，並可使用「重新編輯」保留 boxes 返回標註模式。再次執行時，crop number、頁面與 box 座標皆未改變的結果會直接沿用，不會重複辨識。
